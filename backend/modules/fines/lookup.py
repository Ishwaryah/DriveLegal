...
class FineLookup:
    ...
    def query(self, offence_code: str, vehicle_class: str, state: str, repeat: bool = False, country: str = "IN") -> dict | None:
        ...
        Returns: {amount_inr, repeat_amount_inr, section_ref, source_url, fetched_at, state, country, currency} or None
        """
        conn = sqlite3.connect(self.db_path)
        conn.row_factory = sqlite3.Row
        cursor = conn.cursor()
        
        # Build query based on state flexibility + country
        if state in ("ANY", "ALL"):
            query = """
                SELECT amount_inr, repeat_amount_inr, section_ref, source_url, fetched_at, state, country, currency 
                FROM fines 
                WHERE offence_code = ? AND (vehicle_class = ? OR ? = 'GENERAL' OR vehicle_class = 'ALL')
                  AND country = ?
                LIMIT 1
            """
            cursor.execute(query, (offence_code, vehicle_class, vehicle_class, country))
        else:
            ...
            query = """
                ...
                FROM fines 
                WHERE offence_code = ? AND (vehicle_class = ? OR ? = 'GENERAL' OR vehicle_class = 'ALL')
                  AND (state = ? OR state = 'ALL') AND country = ?
                ORDER BY CASE WHEN state = ? THEN 0 ELSE 1 END
                LIMIT 1
            """
            cursor.execute(query, (offence_code, vehicle_class, vehicle_class, state, country, state))
