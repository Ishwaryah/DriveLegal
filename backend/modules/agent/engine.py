...
class AgentEngine:
    ...
    def __init__(self, fine_lookup, rules_loader, geofencing_engine):
        ...
        self.tool_executor = ToolExecutor(fine_lookup, rules_loader, geofencing_engine, self.hybrid_search)
    ...
    def run(
        ...
        if self.hybrid_search:
            nlp_result = self._run_nlp(user_text)
            if nlp_result:
                return nlp_result
    ...
    def _detect_vehicle(self, text: str) -> str:
        if any(k in text for k in ["bike", "scooter", "motorcycle", "two wheeler", "2w", "helmet"]):
            return "TWO_WHEELER"
